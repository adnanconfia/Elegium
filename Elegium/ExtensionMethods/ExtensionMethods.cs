...
namespace Elegium.ExtensionMethods
{
    public static class ExtensionMethods
    {
        ...
        private static readonly string[] _validImageExtensions = { "jpg", "bmp", "gif", "png", "jpeg" }; //  etc
        private static readonly string[] _validVideoExtensions = { "mp4", "m3u8", "ts", "3gp", "mov", "avi", "wmv" }; //  etc
        private static readonly string[] _validPdfExtensions = { "pdf" }; //  etc
        private static readonly string[] _validAudioExtensions = { "mid", "wma", "aac", "wav", "ogg", "mp3" }; //  etc
        public static string GetFileType(this string ext)
        {
            ext = ext.Replace(".", "");
            if (_validImageExtensions.Contains(ext.ToLower()))
                return "image";
            else if (_validVideoExtensions.Contains(ext.ToLower()))
                return "video";
            else if (_validPdfExtensions.Contains(ext.ToLower()))
                return "pdf";
            else if (_validAudioExtensions.Contains(ext.ToLower()))
                return "audio";
            return "other";
        }
        ...
        public static string ToUserFriendlyTime(this DateTime date)
        {
            if (date == null)
                return string.Empty;
            var today = DateTime.UtcNow.Date.Day;
            var lastWeek = DateTime.UtcNow.AddDays(-7).Day;
            if (today == date.Day)
                return date.ToString("HH:mm tt");
            if (date.Day == today - 1)
                return "Yesterday";
            if (date.Day >= lastWeek && date.Day < today - 1)
                return date.ToString("dddd");
            else
                return date.ToString("MM/dd/yyyy");
        }

        public static async Task<IList<T>> GetPaged<T>(this IQueryable<T> query,
        ...
        public static string GetTaskStatus<T>(this T task) where T : ITask
        {
            if (task == null)
                return string.Empty;
            ...
            if (task.HasDeadline && (task.Deadline.Value - DateTime.UtcNow).Days < 0)
                return "overdue";
        ...
        public static string GenerateRgba(this string backgroundColor, double? backgroundOpacity)
        {
