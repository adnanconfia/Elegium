myApp.controller('FundingRequestsList', function ($scope, $filter, $http, $uibModal, toaster, $ngConfirm, $window, $state) {
    $http.get(root + 'api/FundingFPRequest/GetMyOffers').then(
        (suc) => {
    ...
    $http.get(root + 'api/FundingFPRequest/GetMyRequests').then(
        (suc) => {
        ...
        $ngConfirm({
            ...
            buttons: {
                deleteProject: {
                    ...
                    action: function () {
                        $http.post(root + 'api/FundingFPRequest/TakeFundingRequestAction/', offer).then(function success(response) {
