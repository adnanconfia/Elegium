...
myApp.controller('CalendarController', function ($stateParams, $compile, $filter, $scope, $http,  $window, $uibModal, $timeout, toaster, $ngConfirm, MenuService, $state, uiCalendarConfig) {
    ...
    $scope.eventMenu = $('#event-menu-template').html();
    ...
    $scope.gotoEventProfile = function (eventId) {
        $('.popover').popover('hide');
        $state.go('calendar.calendarprofile', { calendarId: eventId });
    }
    
    $scope.gotoTaskProfile = function (eventId) {
        $('.popover').popover('hide');
        $state.go('tasks.mytasks.taskprofile', { id: $scope.projectId,taskId: eventId });
    }
    ...
    $scope.getCalenderCategories = function () {
        $http.get(root + 'api/CalenderCategories/GetCalenderCategories/' + $scope.projectId).then(resp => {
            //console.log(resp);
    ...
    $scope.fetchEventAndRenderCalendar = function () {
        ...
        clearCalendar();
        $http.get(root + 'api/Calendar/GetEvents/' + parseInt($stateParams.id)).then(function success(response) {
    ...
    $scope.getAllPendingTasks = function () {
        //GetAllPendingTasks
        $http.get(root + 'api/Tasks/GetAllPendingTasks/' + parseInt($stateParams.id)).then(function success(response) {
                ...
                if (!event.isTask) {
                    finalHtml = `<div class="row">
                                    ...
                                    <div class="col-12 text-right">
                                        ...
                                        <a href="javascript:void(0)" class="btn btn-secondary text-white cursor-pointer" title="Detail" ng-click="gotoEventProfile(`+ event.id + `)"><i class="fa fa-ellipsis-h"></i></a>
                                    </div>
                                </div>`;
                }
                else {
                    ...
                    finalHtml = `<div class="row">
                                    ...
                                    <div class="col-12 text-right">
                                        <a href="javascript:void(0)" class="btn btn-secondary text-white cursor-pointer" title="Detail" ng-click="gotoTaskProfile(`+ event.id + `)"><i class="fa fa-ellipsis-h"></i></a>
                                    </div>
                                </div>`;
                }
...
}).controller('CreateEventCtrl', function ($scope, $state, $rootScope, $uibModal, $uibModalInstance, $stateParams, $http, $timeout, toaster, title, event) {
...
}).controller('CalenderCategoriesCtrl', function ($scope, $uibModalInstance, $http, toaster, title, projectId) {

    $scope.title = title;
    $scope.projectId = projectId;
    $scope.CreateOrEdit = "Create";

    $scope.selectedShowOnlyCategory = null;
    ...
    $scope.createCategory = function () {
        if (!$scope.category.Name) {
            toaster.pop({
                type: 'error',
                body: 'Please input category name.',
            });
            return false;
        }

        $scope.category.ProjectId = Number($scope.projectId);

        console.log($scope.category);

        $http.post(root + 'api/CalenderCategories/SaveOrUpdateCalenderCategory', $scope.category).then(resp => {
            console.log(resp);
            if (resp.status == 200) {
    ...
    $scope.showOnlyCategory = function (categoryId) {
        $scope.selectedShowOnlyCategory = categoryId;
    }
    ...
    $scope.cancel = function () {
        $uibModalInstance.dismiss($scope.selectedShowOnlyCategory);
    }
