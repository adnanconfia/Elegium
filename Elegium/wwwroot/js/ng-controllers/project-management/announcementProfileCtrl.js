myApp.controller('announcementProfileController', function ($stateParams, $scope, $http, $state, $window, $window, $uibModal, $timeout) {

    $timeout(function () {

        var announcementId = parseInt($stateParams.announcementId);
        ...
        $http.get(root + 'api/Announcements/GetAnnouncement/' + announcementId).then(resp => {
            $scope.announcement = resp.data.list;
            $scope.announcement.Deadline = new Date($scope.announcement.Deadline);
            $scope.announcement.Created = new Date($scope.announcement.Created);
        ...
        $scope.updateAnnouncement = function () {
            $http.post(root + 'api/Announcements/UpdateAnnouncement', $scope.announcement).then((resp) => {
                console.log(resp);
            }, () => {
            })
        }
        ...
        $scope.getAnnouncementFiles = function () {
            $http.get(root + 'api/Announcements/GetAnnouncementFiles/' + announcementId + '/' + $scope.filesPaging.page + '/' + $scope.filesPaging.size).then(resp => {
        ...
        $scope.openProjectFileViewer = function (file) {
            ...
            modalInstance = $uibModal.open({
                ...
                controller: 'announcementFileViewerCtrl',
                size: 'lg',
                backdrop: 'static',
                resolve: {
                    fileObj: function () {
        ...
        $scope.deleteFile = function ($event, file) {
            ...
            $http.post(root + 'api/Announcements/DeleteAnnouncementFiles/' + file.Id).then(resp => {
                //$scope.getLinks();
        ...
        $scope.getComments = function () {
            //GetComments
            $http.get(root + 'api/Announcements/GetComments/' + announcementId).then(resp => {
                if (resp.data.length > 0)
                    $scope.commentObj.newComment = false;
        ...
        //$http.get(root + 'api/Announcements/GetUserWiseAnnouncements').then((resp) => {
        //    console.log(resp);
        //}, () => {
...
}).controller('announcementFileViewerCtrl', function (fileObj, $http, $scope, $uibModalInstance, projectUsersAndGroups, $state, $timeout, projectId, $stateParams) {
    ...
    $uibModalInstance.rendered.then(function () {
        ...
        $timeout(function () {
            ...
            $scope.nextPrev = function (flag) {
                ...
                $http.get(root + 'api/Announcements/NextPrevFileId/' + $scope.file.Id + '/' + flag + '/' + $scope.file.AnnouncementId).then(resp => {
            ...
            $scope.navigateToFileDetailsView = function (file) {
                $state.go("announcements.announcementprofile.fileProfile", { fileId: file.Id, name: file.Name });
                $scope.cancel();
            }
            //start comments tab
