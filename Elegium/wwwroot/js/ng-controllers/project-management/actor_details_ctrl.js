myApp.controller('actor_details_ctrl', function ($scope, $rootScope, $filter, $http, $timeout, $uibModal, toaster, $ngConfirm, $stateParams, $state, orderByFilter) {
    $scope.eventsTab = [];
        ...
        $timeout(function () {
                    ...
                    if (response.status === 200) {
                        ...
                        angular.forEach(response.data, function (item) {
                            ...
                            if (item.Agency.Id === $scope.Actor.AgencyId) {
                                ...
                                $scope._agency = item.Agency.Name;
                            }
                        });
            ...
            $scope.UpdateActor = (actor) => {
                if (actor.Is_talent == false) {
                    ...
                    actor.Is_talent = false;
                } else {
                    $http.get(root + 'api/ScenesandScript/ActorToTalent/' + parseInt($scope.charId)).then(
                        function success(resp) {
                            $scope.Editinfo = false;
                            $rootScope.GetOnBoarding();
                            $rootScope.getTalent();
                            $rootScope.getactor();
                            $state.go('cast.talent_details', { charId: resp.data })
                        }, function error() { });
                }
                $scope.Editinfo = false;
            ...
            try {
                    ...
                    if ($scope.uploadedFiles) {
                        if ($scope.uploadedFiles.length > 0) {
                            ...
                            angular.forEach($scope.uploadedFiles, function (item) {
                                ...
                                item.ActorId = parseInt($scope.charId);
                ...
                $scope.getLink = () => {
                    ...
                    $http.get(root + 'api/scenesandscript/GetLinksByActorId/' + $scope.charId).then(
                        function success(resp) {
                            $scope.link = resp.data;
                        }, function error() { });
                }
                $scope.getPeriod = () => {
                    $http.get(root + 'api/scenesandscript/GetPeriodsByActorId/' + $scope.charId).then(
                        function success(resp) {
            ...
            var initializeTaskObj = function () {
                ...
                $http.get(root + 'api/Tasks/GetAllTasks/' + $stateParams.id).then(resp => {
                    ...
                    $scope.mytasks = $scope.mytasks.filter(function (item) {
                        ...
                        if (item.ActorID === parseInt($scope.charId)) {
            ...
            $scope.getComments = function () {
                //GetComments
                $http.get(root + 'api/ScenesandScript/GetActorComments/' + parseInt($scope.charId)).then(resp => {
                    if (resp.data.length > 0)
                        $scope.commentObj.newComment = false;
            ...
            $scope.openProjectFileDialog1 = function () {
                ...
                modalInstance.result.then(function () {
                    ...
                    //fetch(data.source).then(res => res.blob())
                    //    .then(blob => {
